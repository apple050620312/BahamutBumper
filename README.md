...
3. 當天尚未自推才回覆「推」。
4. 重新讀取文章，唯一驗證新樓層存在。
5. 再讀一次最新頁面與刪文 token，刪除今天以前最近一則自己的自推回覆；即使前一天流程失敗，也能清掉更早留下的舊推文。
6. 再次確認舊樓層已消失，將結果寫入 `data/status.json`。

文章超過 20 樓進入第 2 頁後，程式仍以最後一頁判斷最新回覆，但會額外唯讀抓取第 1 頁驗證首篇作者與目前登入帳號，避免把「最後一頁看不到 1 樓」誤判成 Cookie 失效。刪除昨天回覆時會使用網站顯示的實際頁碼重新取得該頁；沒有固定最大頁數，也不假設舊回覆一定仍和今天的新回覆在同一頁。
